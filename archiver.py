"""Module 3 - Archivage et nettoyage pour LogAnalyzer Pro."""
...
def verifier_espace_disque(destination: str | os.PathLike[str], espace_min_requis: int) -> int:
    """Verifie l'espace disque disponible avec subprocess.
    ...
    try:
        result = subprocess.run(
            ["df", "-Pk", str(destination_path)],
            check=True,
            capture_output=True,
            text=True,
        )
    except FileNotFoundError as exc:
        raise RuntimeError("La commande systeme 'df' est introuvable.") from exc
    except subprocess.CalledProcessError as exc:
        raise RuntimeError(f"Impossible de verifier l'espace disque: {exc.stderr.strip()}") from exc

    lignes = [ligne for ligne in result.stdout.splitlines() if ligne.strip()]
    if len(lignes) < 2:
        raise RuntimeError("Sortie de 'df' invalide, verification d'espace impossible.")

    colonnes = lignes[-1].split()
    if len(colonnes) < 4:
        raise RuntimeError("Format inattendu pour la sortie de 'df'.")
...
def creer_archive(
    ...
    temp_dir = Path(dossier_temp).expanduser().resolve() if dossier_temp else Path(tempfile.gettempdir())
    temp_dir.mkdir(parents=True, exist_ok=True)

    archive_name = f"backup_{date.today().isoformat()}.tar.gz"
...
def deplacer_archive(
    archive_path: str | os.PathLike[str],
    destination: str | os.PathLike[str],
) -> Path:
    """Deplace l'archive vers le dossier de destination."""
...
def nettoyer_anciens_rapports(
    ...
    """Supprime les rapports JSON plus anciens que la retention."""
...
def estimer_espace_requis(fichiers_logs: list[str | os.PathLike[str]]) -> int:
    ...
    # Petite marge pour couvrir le fichier archive et les metadonnees.
...
def archiver_et_nettoyer(
    ...
    """Orchestre l'archivage des logs et le nettoyage des rapports."""
    dossiers_logs = [str(Path(fichier).expanduser().resolve()) for fichier in fichiers_logs]
    backups_path = Path(dossier_backups).expanduser().resolve()
    rapports_path = Path(dossier_rapports).expanduser().resolve()

    espace_min_requis = estimer_espace_requis(dossiers_logs)
    espace_libre = verifier_espace_disque(backups_path, espace_min_requis)

    archive_temp = creer_archive(dossiers_logs)
...
def construire_parser() -> argparse.ArgumentParser:
    """Construit un parseur CLI minimal pour tester le module seul."""
    parser = argparse.ArgumentParser(description="Archive les logs et nettoie les anciens rapports JSON.")
    ...
    parser.add_argument("--rapports", required=True, help="Dossier contenant les rapports JSON.")
    parser.add_argument(
        "--retention",
        type=int,
...
def main() -> int:
    """Point d'entree CLI du module Neal."""
